...
const ICONS = {
  note: <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>,
...
export function Callout({ type = 'note', title, children }: CalloutProps) {
  ...
  return (
    <div className={`my-8 flex gap-4 rounded-2xl border p-5 sm:p-6 ${styles[type]}`}>
      <div className="shrink-0 mt-0.5">
        {ICONS[type]}
      </div>
      <div className="flex-1 w-full min-w-0">
        {title && <h5 className="m-0 mb-2 text-base font-bold font-heading text-text-primary tracking-wide">{title}</h5>}
        <div className="text-text-secondary [&>p]:m-0 [&>p]:leading-relaxed text-[16px] font-medium">
          {children}
        </div>
      </div>
    </div>
